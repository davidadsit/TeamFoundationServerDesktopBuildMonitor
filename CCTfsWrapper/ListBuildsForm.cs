...
namespace CCTfsWrapper
{
	public partial class ListBuildsForm : Form
	{
		...
		private const string TFS_URL = @"https://dr0-tfs-04/tfs/DefaultCollection";
		...
		private void AnnounceCulprit(string culprit)
		{
			...
			string culpritName = culprit;
			if (_userNames.ContainsKey(culprit))
			{
				culpritName = _userNames[culprit];
			}
			if (!string.IsNullOrEmpty(culprit))
			{
				voiceClass.Speak(string.Format("Aawww crap, {0} broke the freakin build.", culpritName));
			}
		}
