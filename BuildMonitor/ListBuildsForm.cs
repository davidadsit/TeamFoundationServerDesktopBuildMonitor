...
using BuildMonitor.Domain.Notifier;
using CCTfsWrapper;
...
{
	public partial class ListBuildsForm : Form
	{
		...
		public ListBuildsForm()
		{
			InitializeComponent();
			SetupUsers();
			Stream greenBallStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("CCTfsWrapper.Images.GreenBall.gif");
			Stream redBallStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("CCTfsWrapper.Images.RedBall.gif");
			Stream yellowBallStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("CCTfsWrapper.Images.YellowBall.ico");
		...
		{
			INotifier textToSpeachNotifier = new TextToSpeachNotifier("It is unfortunate, but {0} broke the build.");
			textToSpeachNotifier.SetBuildQuality(Domain.BuildStatus.Failure, Domain.BuildStatus.Building, _userNames[culprit]);
		}
		...
		{
			TfsTeamProjectCollection teamProjectCollection = TfsTeamProjectCollectionFactory.GetTeamProjectCollection(new Uri(serverUri));
